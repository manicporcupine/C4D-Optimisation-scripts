...
def objects_are_identical(obj1, obj2):
    if not (obj1 and obj2):
        return False
    if not (obj1.CheckType(c4d.Opolygon) and obj2.CheckType(c4d.Opolygon)):
        return False
...
def relink_instances(doc, all_objects, master):
    for obj in all_objects:
        if obj.CheckType(c4d.Oinstance):
            ...
            real_master = get_master_polygon(ref)
...
def replace_duplicates(doc, all_objects, master, other_masters):
    for obj in list(all_objects):
        if obj.CheckType(c4d.Opolygon) and obj != master and obj not in other_masters:
            if objects_are_identical(master, obj):
                parent = obj.GetUp()
                world_mtx = obj.GetMg()
...
def main():
    ...
    masters = [obj for obj in selected if obj.CheckType(c4d.Opolygon)]
    if not masters:
        gui.MessageDialog("Please select one or more polygonal objects.")
        return
